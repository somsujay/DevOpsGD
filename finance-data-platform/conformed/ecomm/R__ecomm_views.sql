...
CREATE OR REPLACE VIEW {{ conformed_schema }}.MONTHLYSPENDPROFILE AS
SELECT
    C.CUSTOMER_ID,
    C.FIRST_NAME,
    C.LAST_NAME,
...
FROM {{ conformed_schema }}.FACTDAILYTRANSACTION AS F
INNER JOIN {{ clean_schema }}.DIMCUSTOMER AS C
    ON
...
CREATE OR REPLACE VIEW {{ conformed_schema }}.TXNTYPETREND AS
SELECT
    F.TRANSACTION_TYPE,
    DATE_TRUNC('MONTH', F.DATE_KEY) AS MONTH_KEY,
    COUNT(*) AS TRANSACTION_COUNT,
